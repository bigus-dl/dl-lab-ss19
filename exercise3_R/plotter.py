...
class NaturalCubicSpline(AbstractSpline):
    ...
    *not* included as knots.
    ...
    min: float 
        Minimum of interval containing the knots.
    max: float 
        Maximum of the interval containing the knots.if not n_knots:
                n_knots = self._compute_n_knots(n_params)
            knots = np.linspace(min, max, num=(n_knots + 2))[1:-1]
            max, min = np.max(knots), np.min(knots)
    n_knots: positive integer 
...
for i in range(len(data_episode)):
    ...
    plt.xlabel("episode")
    plt.ylabel("reward")
    plt.plot(xnew,ynew, label=data_str[i])
    plt.legend()

plt.show()
fig.savefig("episode.png")

#fplot = data.plot(x='Step', y='Value')
#fig = fplot.get_figure()
#fig.savefig("myplot.png")
#sb.lmplot(x = "Step", y = "Value", data = data, order = 3)
#sb.lineplot(x="Step", y="Value", data=data, err_style='band')
#sb.lineplot(x="Step", y="Value", data=data, err_style='band')
#sb.despine()
#plt.show()